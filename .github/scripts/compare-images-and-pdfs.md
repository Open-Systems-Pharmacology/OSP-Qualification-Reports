...
5. **Reporting**: Posts results as a PR comment (updates existing comment if present) and always writes the report to the workflow run's job summary
...
The workflow is triggered by `pull_request_target` (not `pull_request`). This is
required so that pull requests opened from forks are granted a `GITHUB_TOKEN`
with `pull-requests: write` / `issues: write` permissions, which are needed to
post the comparison comment. With the plain `pull_request` trigger, fork PRs run
with a read-only token and posting a comment fails with
`403 Resource not accessible by integration`.

Because `pull_request_target` runs in the context of the base repository, the
checkout step intentionally checks out the **base** branch (its default) and
never executes code from the PR head. The script only downloads the changed
files through the GitHub API to compare them; it does not run any code from the
pull request.

If the comment still cannot be posted for any reason (for example a read-only
token), the job does **not** fail — the report is written to the workflow run's
job summary instead.
